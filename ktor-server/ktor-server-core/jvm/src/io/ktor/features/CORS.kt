...
/**
 * Configuration for [CORS] feature
 */
public class CorsConfig {
    ...
    private val transformedExposedHeaders = when {
    ...
    public suspend fun intercept(execution: CallExecution) {
        ...
        if (!allowNonSimpleContentTypes) {
            ...
            if (contentType != null) {
                if (contentType.withoutParameters() !in CorsSimpleContentTypes) {
                    execution.respondCorsFailed()
                    return
                }
            }
        ...
        if (transformedExposedHeaders != null) {
            call.response.header(HttpHeaders.AccessControlExposeHeaders, transformedExposedHeaders)
        }
    }
 ...
 */
public val CORS: KtorPlugin<CorsConfig> = createPlugin("CORS", { CorsConfig() }) {
    onCall {
